	...
	methods: {
		...
		changeSearchText(val) {
			this.addressObj={
				...
				address:val.nodeName
			},
			
			this.$store.commit('SET_LONGITUDELINE',val)
			console.log(val,'changeSearchText')
		...
		addressInfoChange(val) {
			...
			this.mapData.routeNodes = arr;
			this.mapData.routeNodesChange=true
			// this.addMarker(arr)
		}
	}
};
...
import posi from '@/static/posi.png'
export default {
	data() {
		return {
			map: null,
	...
	methods: {
		transData(value) { //传输数据 H5有限制，使用ownerInstanceObj.callMethod 代替操作
			console.log(value)
				...
				AMap.plugin(
										...
										function() {
											...
											that.map.on('click', function(e) {
												...
												document.getElementById('latCath').innerText=arr[1]
												that.transData(arr)
												// that.ownerInstanceObj.callMethod('getSearchlines', arr)
												regeoCode(arr)
												that.addMarker(arr)
											})
										}
		...
		addMarker(arr){
			...
			// this.ownerInstanceObj.callMethod('getSearchlines', arr)
			this.map.add(marker)
		...
		searchData(keywords) {
			...
			this.map.plugin('AMap.AutoComplete', function() {
				...
				autoComplete.search(keywords, function(status, result) {
					...
					console.log(that.ownerInstanceObj,"that.ownerInstanceObj")
		...
		updateMap(newValue, oldValue, ownerInstance, instance) {
			...
			if(newValue.routeNodesChange&&newValue.routeNodes.length>0){
				this.addMarker(newValue.routeNodes)
			}
		},
		onClick(event, ownerInstance) {
			console.log(event,"event")
			console.log(ownerInstance,"ownerInstance1111")
		}
	}
}
</script>
